...
/*   Updated: 2024/09/25 19:07:37 by kpoilly          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{	
	...
	if (str.size() <= 0 || value < std::numeric_limits<int>::min() || value > std::numeric_limits<int>::max())
		std::cout << "impossible." << std::endl;
	else
...
static void toFloat(std::string str, long double value)
{
	...
	else if(str.size() <= 0 || ((value < -std::numeric_limits<float>::max() || value > std::numeric_limits<float>::max())))
		std::cout << "impossible." << std::endl;
	else
		{
...
static void toDouble(std::string str, long double value)
{
	...
	else if(str.size() <= 0 || ((value < -std::numeric_limits<double>::max() || value > std::numeric_limits<double>::max())))
		std::cout << "impossible." << std::endl;
	else
...
bool checkError(std::string literal)
{
	...
	for (unsigned long i = 0; i  < literal.size(); i++)
	{
		if (std::isdigit(literal[i]))
			hasNum = true;
		else if (literal[i] > 9 && literal[i] != 'f' && literal[i] != '.' && literal[i] != '-')
			hasChar = true;
	}
