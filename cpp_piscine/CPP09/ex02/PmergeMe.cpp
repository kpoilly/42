...
// Fonction pour calculer le n-ième terme de la suite de Jacobsthal
...
void slice(std::deque<int>& arr, int left, int right) {
  if (right - left <= 1) {
    return; // Un seul élément ou une paire, déjà triée
  }

  int mid = std::min(left + jacobsthal((right - left) / 2), right - 1);
  slice(arr, left, mid);
  slice(arr, mid + 1, right);
  merge(arr, left, mid, right); // Call the merge function
}
